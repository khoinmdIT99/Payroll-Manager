...
namespace Payroll_Manager.Areas.Identity.Pages.Account
{
    ...
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var data = await _userManager.FindByEmailAsync(Input.Email);
                if (data == null)
                {
                    ModelState.AddModelError("", "Trùng Email");
                }
                // This doesn't count login failures towards account lockout
                // To enable password failures to trigger account lockout, set lockoutOnFailure: true
                var result = await _signInManager.PasswordSignInAsync(data, Input.Password, Input.RememberMe, lockoutOnFailure: false);
                if (result.Succeeded)
                {
